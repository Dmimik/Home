...
export function Footer() {
  ...
  return (
    <footer style={{ background: "rgba(8,12,18,0.98)", borderTop: "1px solid var(--border)" }}>
      <div className="container mx-auto max-w-6xl px-3 sm:px-4 py-8 sm:py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {resources.map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="footer-link footer-link-muted">
                    {item.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 min-w-0 w-full sm:w-auto">
              <input
                ...
                className="flex-1 min-w-0 px-3 py-2 rounded-lg text-sm outline-none mono"
                style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}
              />
              <button
                type="button"
                className="px-3 py-2 rounded-lg shrink-0 self-stretch sm:self-auto flex items-center justify-center"
        ...
        <div
          className="mt-10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 text-center sm:text-left"
          style={{ borderTop: "1px solid var(--border)" }}
        >
          <p className="text-xs mono" style={{ color: "var(--text-3)" }}>
            {mounted && <>&copy; {year} CoinVault Protocol. All rights reserved.</>}
          </p>
            <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-xs mono" style={{ color: "var(--text-3)" }}>
